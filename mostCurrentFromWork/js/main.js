		...
		this.move = function(timestamp) { // the ball moves now when you click but now you don't need a click event to trigger the move
			...
			// $(window).on('click', function(){
				...
				function motion () { // collision should take in a paddle object as an argument
					...
					if (ballThis.ballX < rightPaddle.x + 25 &&
		   				ballThis.ballX + 20 > rightPaddle.x &&
		   				ballThis.ballY < rightPaddle.y + rightPaddle.paddleHeight &&
		   				ballThis.ballHeight + ballThis.ballY > rightPaddle.y) {
						ballThis.ballDY =- ballThis.ballDY; // clarify this 
					}
				...
				motion()
			//}) // end of moves keydown
			
		} // end of move
		this.collidesWith = function(otherObject) {
		  var width = 2 * this.ballRadius;
		  var height = 2 * this.ballRadius;
		  var translatedX = this.ballX - this.ballRadius;
		  var translatedY = this.ballY - this.ballRadius;
		  
		  console.log('inside collidesWidth width: ', width)

		  if (translatedX < rightPaddle.x + rightPaddle.paddleWidth &&
		     translatedX + width > rightPaddle.x &&
		     translatedY < rightPaddle.y + rightPaddle.paddleHeight &&
		     height + translatedY > rightPaddle.y) {
		      // collision detected!
